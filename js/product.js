document.addEventListener('DOMContentLoaded',()=>{
  const id=new URLSearchParams(location.search).get('id') || 'nicer-dicer';
  const p=Store.product(id);
  if(!p){location.href='index.html';return}
  document.title=p.name+' — Твій Дім';
  const media=p.image
    ? `<img src="${p.image}" alt="${p.name}" style="width:100%;height:100%;object-fit:contain;padding:30px;display:block" onerror="this.style.display='none';this.parentElement.insertAdjacentHTML('beforeend','<span>${p.emoji || '•'}</span>')">`
    : (p.emoji || '•');
