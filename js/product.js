document.addEventListener('DOMContentLoaded',()=>{
  ...
  const main=imgs.length
    ? `<img id="galleryMainImage" src="${imgs[0]}" alt="${p.name}" onerror="this.style.display='none';const f=this.parentElement.querySelector('.gallery-fallback');if(f)f.style.display='grid'">${fallback}`
    : fallback;
  ...
  document.querySelectorAll('.gallery-thumb').forEach(btn=>btn.addEventListener('click',()=>{
    ...
    const fallbackEl=document.querySelector('.gallery-main .gallery-fallback');
